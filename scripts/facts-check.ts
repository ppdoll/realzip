...
const PICK: [string, [string, number][], string, string | null][] = [
  ...
  // 경기 감사에서 새로 잡은 것 — K-apt 등록명이 거래명에 "아파트" 를 붙인 정도라면
  // 거래명을 통째로 품는다. 앞부분만 겹치는 후보는 특징적인 낱말을 잃은 다른 단지다.
  // 남양주 다산동 다산롯데캐슬 170건이 708세대(롯데낙천대)에 붙어 회전율 30.4% 였다.
  ['다산롯데캐슬 — 품는 쪽 (길이만 보면 짧은 쪽을 집는다)', [['다산 롯데캐슬아파트', 1186], ['다산롯데아파트', 708]], '다산롯데캐슬', '다산 롯데캐슬아파트'],
  // 오산 부산동 — 2차/2단지/1단지가 한 동에 있고 "2차" 를 품는 것은 하나다 (57건, 이전엔 null)
  ['오산시티자이2차 — 차수를 품는 쪽', [['오산시티자이2차아파트', 1090], ['오산시티자이2단지아파트', 941], ['오산시티자이1단지', 1099]], '오산시티자이2차', '오산시티자이2차아파트'],
  // 근거가 얇으면 포기해야 한다 — 틀린 값을 보여주는 것보다 값이 없는 게 낫다
  ['수서 — 1자 차이로는 못 가린다', [['일원동 수서아파트', 720], ['수서1-1단지아파트', 2214]], '수서', null],
  // 품는 후보가 하나여도 **거래명에 없는 번호를 얹었으면** 다른 단지다.
  // 대치동 910-4 풍림아이원아파트를 19세대 등록(1.2단지)에 붙이면 회전율이 폭발한다.
  ['풍림아이원 — 번호를 새로 얹은 후보는 거부', [['대치풍림아이원아파트 1.2단지', 19], ['대치풍림아이원3.4차아파트', 75]], '풍림아이원아파트', null],
...
// -- 준공년도 대조 --
// 실거래 건축년도와 K-apt 사용승인 연도는 실측 6,919단지 중 98.05%가 정확히 일치한다.
// 3년 이상 어긋난 0.97%는 눈으로 본 15건이 전부 오조인이었다 (짧은 이름이 긴 이름에 포함돼 붙음).
type YCand = { umdNm: string; jibun: string | null; kaptName: string; useDate?: string | null };
const ym = (pool: YCand[], jibun: string | null, aptNm: string, buildYear: number | null) =>
  matchKapt(pool, { umdNm: pool[0].umdNm, jibun, aptNm, buildYear })?.kaptName ?? null;

// 막아야 하는 것 — 전부 경기·서울 실제 데이터다
check(
  ym([{ umdNm: '평내동', jibun: '661', kaptName: 'e편한세상 평내 메트로원', useDate: '20220624' }],
      '661', '평내호평역대명루첸포레스티움', 2020) === null,
  '지번은 맞아도 이름 다르고 2년 차이면 거부 (남양주 평내동 661)',
);
check(
  ym([{ umdNm: '광명동', jibun: '782', kaptName: '광명팰리스필2차', useDate: '20070516' }],
      '777', '팰리스필', 2004) === null,
  '준공 3년 차이면 거부 (팰리스필 2004 vs 2차 2007)',
);
check(
  ym([{ umdNm: '개포동', jibun: '1', kaptName: '개포자이아파트', useDate: '20040101' }],
      '1', '개포자이르네', 2022) === null,
  '개포자이르네(2022)는 개포자이아파트(2004)가 아니다',
);
check(
  ym([{ umdNm: '서초동', jibun: '1', kaptName: '롯데캐슬헤론', useDate: '20060101' }],
      '9', '롯데아파트', 1998) === null,
  '롯데아파트(1998)는 롯데캐슬헤론(2006)이 아니다',
);

// 붙어야 하는 것
check(
  ym([{ umdNm: '평내동', jibun: '661', kaptName: 'e편한세상 평내 메트로원', useDate: '20220624' }],
      '667', 'e편한세상평내메트로원', 2022) === 'e편한세상 평내 메트로원',
  '지번이 달라도 이름·연도가 맞으면 붙는다',
);
check(
  ym([{ umdNm: '풍덕천동', jibun: '664', kaptName: '초입마을삼익풍림동아', useDate: '19941201' }],
      '664', '삼익아파트', 1994) === '초입마을삼익풍림동아',
  'K-apt 가 세 브랜드를 한 단지로 등록한 경우 (용인 수지 삼익·풍림·동아)',
);
check(
  ym([{ umdNm: '백현동', jibun: '1', kaptName: '더샵판교퍼스트파크', useDate: '20150301' }],
      '1', 'THESHARP판교퍼스트파크', 2015) === '더샵판교퍼스트파크',
  '이름이 전혀 달라도 지번·연도가 같으면 붙는다',
);
check(
  ym([{ umdNm: '가', jibun: '1', kaptName: '어느아파트', useDate: '20201230' }],
      '1', '전혀다른이름', 2021) === '어느아파트',
  '1년 차이는 통과 (승인 지연은 흔하다)',
);
check(
  ym([{ umdNm: '백현동', jibun: '1', kaptName: '더샵판교퍼스트파크', useDate: null }],
      '1', 'THESHARP판교퍼스트파크', null) === '더샵판교퍼스트파크',
  '연도를 모르면 대조하지 않는다 (근거 없음 ≠ 어긋남)',
);
// 지번 후보를 물려도 이름으로 다시 찾아야 한다 — 지번이 잘못 신고된 경우가 있다
check(
  ym([
    { umdNm: '평내동', jibun: '661', kaptName: 'e편한세상 평내 메트로원', useDate: '20220624' },
    { umdNm: '평내동', jibun: '191', kaptName: '평내호평역 대명루첸포레스티움아파트', useDate: '20201101' },
  ], '661', '평내호평역대명루첸포레스티움', 2020) === '평내호평역 대명루첸포레스티움아파트',
  '지번 후보를 물려도 이름으로 제 짝을 찾는다',
);

// 괄호 안 숫자는 지번·동 번호라 단지 번호가 아니다 (실측: 지번·연도가 맞는데
// 숫자 충돌로 물리던 40건 중 절반이 이 오탐이었다)
check(
  ym([{ umdNm: '성산동', jibun: '667', kaptName: '월드컵아이파크1단지', useDate: '20090101' }],
      '667', '월드컵아이파크(667)', 2009) === '월드컵아이파크1단지',
  '괄호 안 지번은 단지 번호로 세지 않는다',
);
check(
  ym([{ umdNm: '가', jibun: '1060', kaptName: '동익미라벨39단지', useDate: '20130101' }],
      '1060', '동익미라벨(1060)', 2013) === '동익미라벨39단지',
  '괄호 지번 무시 (동익미라벨)',
);
// 그래도 진짜 단지 번호가 다르면 거부해야 한다
check(
  ym([{ umdNm: '압구정동', jibun: '513', kaptName: '압구정한양3단지', useDate: '19790101' }],
      '513', '한양5', 1979) === null,
  '지번·연도가 같아도 단지 번호가 다르면 거부 (한양5 vs 한양3단지)',
);
check(
  ym([{ umdNm: '길음동', jibun: '1', kaptName: '길음래미안3차', useDate: '20070101' }],
      '1', '길음뉴타운6단지(래미안)', 2007) === null,
  '6단지는 3차가 아니다',
);
// 이름·지번이 완전히 같으면 연도 차이를 무시한다 (한쪽 자료의 오기)
check(
  ym([{ umdNm: '불광동', jibun: '645', kaptName: '불광롯데캐슬', useDate: '20130101' }],
      '645', '불광롯데캐슬', 1998) === '불광롯데캐슬',
  '이름·지번이 같으면 연도 15년 차이도 같은 건물 (은평 불광롯데캐슬)',
);
